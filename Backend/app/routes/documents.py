...
GET  /api/documents/{id}/content        → raw file bytes (PDF as-is, DOCX → PDF)
"""
...
def _docx_bytes_to_pdf(docx_bytes: bytes) -> bytes:
    ...
    2. docx2pdf              – uses Word on Windows/Mac, LibreOffice on Linux
    3. python-docx + reportlab – pure-Python plain-text fallback
    """
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp:
                ...
                result = subprocess.run(
                    [lo, "--headless", "--convert-to", "pdf", "--outdir", tmp, src],
...
def list_import_sessions():
    ...
    try:
        ...
        cur = con.execute(
            ...
            "FROM import_sessions ORDER BY id DESC"
        )
        rows = cur.fetchall()
        con.close()
        return {
            "sessions": [
                {"id": r[0], "subject_filter": r[1], "fetched_count": r[2], "created_at": r[3]}
                for r in rows
            ]
        }
    except Exception as e:
...
def get_import_documents(import_id: int):
    ...
    try:
        con = _get_db()
        row = con.execute(
            "SELECT id, subject_filter FROM import_sessions WHERE id = ?", (import_id,)
        ).fetchone()
        if not row:
            con.close()
        ...
        cands = con.execute(
            ...
            "FROM candidate_documents WHERE import_session_id = ? ORDER BY id",
            (import_id,),
        ).fetchall()

        jd_row = con.execute(
            "SELECT id, filename, mime_type FROM documents "
            "WHERE import_session_id = ? AND doc_type = 'jd' LIMIT 1",
            (import_id,),
        ).fetchone()
        con.close()
...
def get_document_info(doc_id: int):
    ...
    try:
        con = _get_db()
        row = con.execute(
            "SELECT id, filename, mime_type, doc_type FROM documents WHERE id = ?", (doc_id,)
        ).fetchone()
        con.close()
        if not row:
...
def get_document_content(
    ...
    """Stream document bytes. PDFs served as-is; DOCX converted to PDF when ?as_pdf=true."""
    try:
        con = _get_db()
        row = con.execute(
            "SELECT filename, mime_type, file_data FROM documents WHERE id = ?", (doc_id,)
        ).fetchone()
        con.close()
        ...
        # Correct octet-stream MIME for Word files
